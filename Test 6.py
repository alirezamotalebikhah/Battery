...
def run_optimization(cycle_limit):
    ...
    #     fix loads
        7: {"name":"Fridge" , "ratedpower" : 0.2 , "preferd":range(1,97) , "shift":range(1,97) , "must":96},
        8: {"name":"Lighting" , "ratedpower" : 0.5 , "preferd":range(1,97) , "shift":range(1,97) , "must":96},
    }
    model.pflex = Param(model.A, initialize={a: appiliance[a]["ratedpower"] for a in model.A},
                        doc="Rated power of appiliance")
    ...
    # Battery Define
    model.BessCapacity = Param(initialize=5, doc="Bess capacity")
    ...
    model.P_max_net=Param(initialize=10, doc="max power of Net")
    ...
    def cyleconstraints(model):
        return model.N_cycle <= cycle_limit
...
def run_scenarios():
    scenarios = [
        {"name": "scenario1", "cycle_limit": 0.5},
        {"name": "scenario2", "cycle_limit": 1.0},
        {"name": "scenario3", "cycle_limit": 2.0}
    ]

    for scenario in scenarios:
        cycle_limit = scenario["cycle_limit"]
        output_file = f"hems_results_{scenario['name']}.csv"

        # اجرای بهینه‌سازی برای سناریو
        model = run_optimization(cycle_limit)
        Q_initial = model.BessCapacity.value
        dt = 0.25
        P_dis = [model.P_dis[t].value for t in model.T]
        P_ch = [ model.P_ch[t].value for t in model.T ]
        E_disch = sum(P_dis)*dt
        E_ch = sum(P_ch)*dt
        DOD = E_disch/Q_initial if Q_initial > 0 else 0
        FEC = sum(P_dis)*dt / Q_initial if Q_initial > 0 else 0
        beta_crate = model.alpha.value * model.crate.value + model.gamma.value
        beta_DOD = model.cfix.value * (DOD**0.6)**3 + model.zeta.value
        Q_loss_cycle = beta_crate * beta_DOD * (FEC**0.5)
        SOH = Q_initial  - Q_loss_cycle
        ...
        data = {
            ...
            'N_cycle': [model.N_cycle.value for t in model.T],
        ...
        data['Q_loss_cyc'] = [Q_loss_cycle for _ in model.T]
        data['SOH'] = [SOH for _ in model.T]
        data['FEC'] = [FEC for _ in model.T]
        data['DOD'] = [DOD for _ in model.T]
        for a in model.A:
